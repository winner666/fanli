...
function addUserShop() {
	...
	var phone = $('#phone').val(); //用户名手机号
	var pass = $('#pass').val(); //输入密码
	var repass = $('#repass').val(); //确认密码
	var address = $('#address').val(); //详细地址
	var call = $('#call').val(); //服务热线
	var remark = $('#remark').val(); //店铺简介
	var shoptype = $('#shopType').val(); //店铺类型

	console.log(areaId, shoptype)
	if(areaId == -1 || shopName == '' || adShow == '' || phone == '' || pass == '' || repass == '' || address == '') {
		alert('请将信息填写完整');
		return false;
	}
